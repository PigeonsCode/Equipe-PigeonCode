...
class FormCriaProjeto(FlaskForm):
    project_name = StringField("Nome do projeto",validators=[DataRequired(),Length(min=4,max=12)])
...
class FormUserAvalia(FlaskForm):
    ...
    incremento_do_produto_p1 = RadioField("Como foi a satisfação do cliente em relação à entrega do incremento do produto ao final do Sprint?",
                          ...
                          (1,"Cliente extremamente insatisfeito e risco de desistência. Foram feitas apenas críticas extensas. O feedback foi completamente negativo e a continuação do projeto está ameaçada.")],coerce=int,validators=[DataRequired()])
    ...
    incremento_do_produto_p2 = RadioField("O incremento do produto atendeu a todas as tarefas definidas na versão final do Sprint Backlog?",
                          ...
                          (1,"Não, nenhuma tarefa foi concluída como o planejado e o valor do produto entregue não foi cumprido, ameaçando a completude das próximas Sprints.")],coerce=int,validators=[DataRequired()])


    incremento_do_produto_p3 = RadioField("O incremento trouxe avanços significativos para atingir a meta do projeto na totalidade?",
                          choices=[
                          ...
                          (1,"Não, o incremento não coincide com o prometido e ameaça fortemente a conclusão do projeto conforme a meta e tempo planejados. Há possibilidade de não conseguir entregar ao final de todas as Sprints.")],coerce=int,validators=[DataRequired()])


    incremento_do_produto_p4 = RadioField("O produto entregue está conforme os critérios definidos pela Definição de Pronto?",
                          choices=[
                          ...
                          (1,"Não, os itens do DoD não foram seguidos e a qualidade do incremento foi muito comprometida, levando bugs, erros e outras complicações para a próxima Sprint.")],coerce=int,validators=[DataRequired()])
    
    daily_scrum_p1 = RadioField("Nas reuniões de Daily Scrum, foram feitas perguntas para acompanhar o que já foi feito, quais serão os próximos passos e se há impedimentos, para cada membro do Time de Desenvolvimento?",
                          choices=[
                          ...
                          (1,"Não, não foi utilizado nenhuma metodologia de perguntas para acompanhar o progresso, planejamento e impedimentos do time, ocasionando em extrema incerteza sobre o andamento do trabalho.")],coerce=int,validators=[DataRequired()])
    ...
    daily_scrum_p2 = RadioField("Como você avalia a eficácia das reuniões de Daily Scrum? (Considere se elas mantiveram o foco de acompanhar o progresso do Sprint.)",
                          ...
                          (1,"Negativa, nenhuma parte conseguiu se comunicar sobre seus progressos, planos e impedimentos, ocasionando no time estando completamente perdido quanto ao avanço do time.")],coerce=int,validators=[DataRequired()])
    
    sprint_retrospective_p1 = RadioField("Durante a reunião da Sprint Retrospective, os colaboradores da equipe citaram o que ocorreu bem, onde detectaram problemas e as áreas de melhoria?",
                          choices=[
                          ...
                          (1,"Não, a equipe não discutiu abertamente os pontos positivos, problemas e nem as áreas de melhoria.")],coerce=int,validators=[DataRequired()])
    
    sprint_retrospective_p2 = RadioField("Todos os integrantes (Product Owner, Scrum Master e Dev Team) conseguiram apresentar suas perspectivas de forma aberta e construtiva?",
                          choices=[
                          ...
                          (1,"Não, os integrantes não tiveram abertura para expressar suas opiniões de forma construtiva.")],coerce=int,validators=[DataRequired()])
    
    sprint_retrospective_p3 = RadioField("Houve uma boa concordância e uniformidade nas respostas de cada um? Ou cada membro pareceu ter uma perspectiva diferente do que deu ou não, certo?",
                          choices=[
                          ...
                          (1,"Não, os membros do grupo demonstraram perspectivas totalmente distintas sobre o que foi bem realizado e sobre os pontos que podem melhorar.")],coerce=int,validators=[DataRequired()])
    
    sprint_retrospective_p4 = RadioField("Houve uma participação ativa de todos os membros na reunião?",
                          choices=[
                          ...
                          (1,"Não, nenhum membro do grupo se mostrou proativo a participar e compartilhar suas perspectivas e contribuir ativamente.")],coerce=int,validators=[DataRequired()])
    
    burnup_p1 = RadioField("Como você avalia a utilização do gráfico de Burnup para o acompanhamento da conclusão das Sprints ao longo do projeto todo?",
                          choices=[
                          ...
                          (1,"O gráfico de Burnup não foi utilizado de forma adequada, dificultando a visualização do progresso das Sprints e comprometendo o acompanhamento do avanço do projeto.")],coerce=int,validators=[DataRequired()])
    ...
    sprint_backlog_p1 = RadioField("As tarefas definidas no Sprint Backlog decorrem dos respectivos itens no Product Backlog?",
                          ...
                          (1,"Não, nenhuma das tarefas decorrem das User Stories corretas, impossibilitando que o time entregue o MVP referente a Sprint.")],coerce=int,validators=[DataRequired()])
    ...
    sprint_backlog_p2 = RadioField("A velocidade é definida pela quantidade média de Story points concluídos em uma Sprint. Com base nessa definição, os itens do Sprint Backlog respeitam a métrica do time?",
                          ...
                          (1,"Não, a média não foi considerada em nenhum momento e a quantidade de Story Points está muito acima ou muito abaixo da média, arriscando a entrega do produto inteiro e a qualidade das próximas Sprints.")],coerce=int,validators=[DataRequired()])
    ...
    sprint_backlog_p3 = RadioField("A Sprint possui um MVP (Produto mínimo viável) bem estabelecido?",
                          ...
                          (1,"Não, o MVP não é claro por meio da escolha das tarefas e não está anotado em nenhum lugar. A equipe não sabe o que deve ser entregue.")],coerce=int,validators=[DataRequired()])
    ...
    sprint_backlog_p4 = RadioField("As tarefas selecionadas possuem prioridade, User Story e estimativas (Story Points) bem definidas?",
                          ...
                          (1,"Não, a lista está despadronizada e segue algo único e pensado pela própria equipe ao invés das normas especificadas.")],coerce=int,validators=[DataRequired()])
    ...
    dod_p1 = RadioField(" A Definition of Done têm critérios de aceitação bem definidos para considerar um item pronto?",
                          ...
                          (1,"Não, não foram definidos critérios de aceitação sobre a DoD, ocasionando total falta de consenso no que seria considerado pronto")],coerce=int,validators=[DataRequired()])
    ...
    dod_p2 = RadioField("A Definition of Done foi acordada formalmente entre o Product Owner e o Time de Desenvolvimento?",
                          ...
                          (1,"Não, apenas uma das partes decidiu os critérios, a DoD não está documentada, sendo lembrada por meio de acordos verbais, gerando grandes chances do filtro ser completamente ignorado e vários itens não prontos serem aprovados.")],coerce=int,validators=[DataRequired()])
    ...
    dod_p3 = RadioField("O DoR foi seguido corretamente pela equipe de desenvolvimento?",
                          ...
                          (1,"Não, não houve compreensão e não foi possível seguir os critérios para definir itens como preparado corretamente.")],coerce=int,validators=[DataRequired()])
    ...
    sprint_review_p1 = RadioField("As partes interessadas forneceram feedback sobre o incremento apresentado?",
                          ...
                          (1,"Não, as partes interessadas não forneceram feedback sobre o incremento apresentado, resultando em incerteza sobre o nível de satisfação.")],coerce=int,validators=[DataRequired()])
    ...
    sprint_review_p2 = RadioField("Os resultados do Sprint estão bem alinhados à visão do produto e ao objetivo definido? Houve impedimentos significativos que afetaram essas entregas?",
                          ...
                          (1,"Não, os resultados do Sprint estão desalinhados com a visão do produto e com o objetivo definido. Houve fortes impedimentos que comprometeram significativamente a entrega.")],coerce=int,validators=[DataRequired()])
    ...
    sprint_review_p3 = RadioField("No caso de itens não concluídos, o Product Backlog foi atualizado de acordo?",
                          ...
                          (1,"Não, o Product Backlog não foi atualizado, afetando a transparência e o planejamento das próximas Sprints.")],coerce=int,validators=[DataRequired()])
    ...
    sprint_review_p4 = RadioField("Com base no ritmo do time e nas funcionalidades restantes, a previsão de entrega para o Produto Final ainda é realista?",
                          ...
                          (1,"Não, a previsão de entrega não parece mais realista, exigindo um replanejamento e revisão do trabalho acerca do produto.")],coerce=int,validators=[DataRequired()])
    ...
    burndown_p1 = RadioField("O gráfico apresenta uma linha de progresso ideal, dedicada a guiar o tempo em que as tarefas são concluídas até o final do Sprint?",
                          ...
                          (1,"Não, o gráfico não apresenta uma linha de progresso ideal clara, impedindo que o time tenha uma referência eficaz para acompanhar o andamento das tarefas até o final do Sprint.")],coerce=int,validators=[DataRequired()])
    ...
    burndown_p2 = RadioField("Ao longo do Sprint, no BurnDown Chart, como você avalia a linha de conclusão do trabalho real com a linha de conclusão ideal estabelecida?",
                          ...
                          (1,"A linha de conclusão do trabalho real apresentou grandes diferenças em relação à linha ideal, mostrando que o planejamento do Sprint não foi seguido corretamente e que o time enfrentou dificuldades para concluir as tarefas no prazo.")],coerce=int,validators=[DataRequired()])
    ...
    product_backlog_p4 = RadioField("Houve mudanças decorrentes de conversas com o cliente? Se sim, o Product Backlog foi atualizado corretamente a partir do novo feedback?",
                          ...
                          (1,"O Product Backlog não foi alterado, mesmo com atualizações pendentes. Permaneceu o mesmo até o fim da Sprint. ")],coerce=int,validators=[DataRequired()])
    ...
    dor_p2 = RadioField("A Definition of Ready foi acordada formalmente entre o Product Owner e o Time de Desenvolvimento?",
                          ...
                          (1,"Não, apenas uma das partes decidiu os critérios, a DoR não está documentada, sendo lembrada por meio de acordos verbais, gerando grandes chances do filtro ser completamente ignorado e vários itens não preparados serem aprovados.")],coerce=int,validators=[DataRequired()])
    ...
    sprint_planning_p2 = RadioField("As tarefas foram planejadas devidamente?",
                          ...
                          (1,"Não, as tarefas selecionadas não levaram em conta quais itens deveriam ser priorizados do Product Backlog, muito menos os itens entregáveis e os riscos e dependências entre eles; resultando em uma Sprint com promessas confusas.")],coerce=int,validators=[DataRequired()])
    ...
    sprint_planning_p3 = RadioField("Como você avalia a forma como foi feita a divisão das tarefas?",
                          ...
                          (1,"Ruim, as tarefas foram mal distribuídas, muitos membros ficaram sobrecarregados enquanto outros tiveram pouca participação e ficaram ociosos.")],coerce=int,validators=[DataRequired()])
    ...
    sprint_planning_p4 = RadioField("Os itens selecionados do Product Backlog atendiam ao DoR definido antes do início do planejamento?",
                          ...
                          (1,"Não, os itens selecionados do Product Backlog não atenderam ao DoR definido, comprometendo a clareza e o andamento do Sprint.")],coerce=int,validators=[DataRequired()])
    ...
    story_point_p1 = RadioField("As Story Points foram definidas de acordo com as estimativas de esforço entre os membros do Time de Desenvolvimento?",
                          ...
                          (1,"Não, as Story Points não foram definidas de acordo com as estimativas do Time de Desenvolvimento, sendo determinadas sem consenso ou alinhamento entre os membros.")],coerce=int,validators=[DataRequired()])
    ...
    story_point_p2 = RadioField(" Como você avalia a utilização das métricas de estimativa em seus Backlogs?",
                          ...
                          (1,"As métricas de estimativa foram pouco e/ou mal utilizadas, dificultando o planejamento e a priorização das tarefas do Backlog, atrapalhando o acompanhamento do progresso do Sprint.")],coerce=int,validators=[DataRequired()])
